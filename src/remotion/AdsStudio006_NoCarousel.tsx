...
// Scene durations in frames (30fps) - Scene 3 (Carousel) removed
const INTRO_DURATION = 137; // 4.57 seconds
const TITLE_DURATION = 90; // 3 seconds
// CAROUSEL_DURATION removed
...
// Total duration for fade out calculation (without Carousel)
const TOTAL_DURATION = INTRO_DURATION + TITLE_DURATION + STACKED_DURATION + STACKED2_DURATION + STACKED3_DURATION + OUTRO1_DURATION + OUTRO2_DURATION;

export const AdsStudio006_NoCarousel: React.FC = () => {
  // Calculate scene start times (Scene 3 removed, scenes renumbered)
  const scene1Start = 0;
  const scene2Start = INTRO_DURATION;
  // Scene 3 (Carousel) removed - now goes directly to Stacked Videos 1
  const scene3Start = scene2Start + TITLE_DURATION; // Stacked videos 1 (was Scene 4)
  const scene4Start = scene3Start + STACKED_DURATION; // Stacked videos 2 (was Scene 5)
  const scene5Start = scene4Start + STACKED2_DURATION; // Stacked videos 3 (was Scene 6)
  const scene6Start = scene5Start + STACKED3_DURATION; // Outro 1 (was Scene 7)
  const scene7Start = scene6Start + OUTRO1_DURATION; // Outro 2 (was Scene 8)
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#0a0a0a" }}>
      ...
      {/* Scene 3 (Carousel) REMOVED */}

      {/* Scene 3: Stacked Videos 1 (Top and Bottom) - was Scene 4 */}
      <Sequence from={scene3Start} durationInFrames={STACKED_DURATION}>
        <StackedVideoScene />
      </Sequence>

      {/* Scene 4: Stacked Videos 2 (Top and Bottom) - was Scene 5 */}
      <Sequence from={scene4Start} durationInFrames={STACKED2_DURATION}>
        <StackedVideoScene2 />
      </Sequence>

      {/* Scene 5: Stacked Videos 3 (Top and Bottom) - was Scene 6 */}
      <Sequence from={scene5Start} durationInFrames={STACKED3_DURATION}>
        <StackedVideoScene3 />
      </Sequence>

      {/* Scene 6: Outro 1 - was Scene 7 */}
      <Sequence from={scene6Start} durationInFrames={OUTRO1_DURATION}>
        <AbsoluteFill>
          <Video
            muted
            src={staticFile("ads-studio-006-outro1.mp4")}
            style={{
              width: "100%",
              height: "100%",
              objectFit: "cover",
      ...
      {/* Scene 7: Outro 2 - was Scene 8 */}
      <Sequence from={scene7Start} durationInFrames={OUTRO2_DURATION}>
        <AbsoluteFill>
          <Video
            muted
            src={staticFile("ads-studio-006-outro2.mp4")}
            style={{
              width: "100%",
              height: "100%",
              objectFit: "cover",
